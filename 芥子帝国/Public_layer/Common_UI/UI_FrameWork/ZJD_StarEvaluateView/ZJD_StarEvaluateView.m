...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self) {
        ...
        if (defaultImage) {
            ...
            self.defaultImage = [UIImage imageNamed:@"starh"];
        }
        
        if (lightImage) {
            self.lightImage = lightImage;
        } else {
            self.lightImage = [UIImage imageNamed:@"star"];
        }
        
        for (NSInteger j = 0; j < 5; j++) {
            UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(j* (starWidth + space), 0, starWidth, self.height)];
            
            btn.userInteractionEnabled = isCanTap;
            btn.tag = j + 1;
...
- (void)starTapBtn:(UIButton *)btn{
    ...
    if ([self.delegate respondsToSelector:@selector(starView:andStarIndex:)]) {
        [self.delegate starView:self andStarIndex:btn.tag];
    }
