...
- (void)createToolbar
{
    ...
    _toolbar.limitNum = _limitNum;
    _toolbar.selectNum = _selectNum;
    _toolbar.sourceFromAblum = _sourceFromAblum;
...
{
    if ([self.delegate respondsToSelector:@selector(clickToolBarALAsset:isSelect:)]) {
        [self.delegate clickToolBarALAsset:asset isSelect:isSelect];
    }
}

- (BOOL)shouldClickToolBarALAsset:(ALAsset *)asset isSelect:(BOOL)isSelect
{
    if ([self.delegate respondsToSelector:@selector(shouldClickToolBarALAsset:isSelect:)]) {
        return [self.delegate shouldClickToolBarALAsset:asset isSelect:isSelect];
    }
    return YES;
}
...
- (void)finishToolBar
{
    ...
    if ([self.delegate respondsToSelector:@selector(finishToolBar)]) {
        return [self.delegate finishToolBar];
    }
}
