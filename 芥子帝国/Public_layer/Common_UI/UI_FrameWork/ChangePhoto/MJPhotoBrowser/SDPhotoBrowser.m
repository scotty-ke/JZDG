...
- (void)setupToolbars
{
    ...
    indexLabel.bounds = CGRectMake(0, 0,SCREEN_WIDTH,64);
    ...
    indexLabel.text = [NSString stringWithFormat:@"%@",self.titleArray[0]];
    ...
    textView.frame = CGRectMake(0,SCREEN_HEIGHT-80,SCREEN_WIDTH,80);
    textView.backgroundColor = [UIColor lightGrayColor];
    textView.alpha = 0.8;
    textView.editable = NO;
    textView.textColor = [UIColor whiteColor];
    
    NSString *bootmString = [NSString stringWithFormat:@"1/%ld 病情摘要：%@",self.imageCount,self.commdtion];
    NSMutableAttributedString *noteStr = [[NSMutableAttributedString alloc] initWithString:bootmString];
    NSString *integer = [NSString stringWithFormat:@"1/%ld",self.imageCount];
    NSRange redRange = NSMakeRange([[noteStr string] rangeOfString:integer].location, [[noteStr string] rangeOfString:integer].length);
    [noteStr addAttribute:NSFontAttributeName value:TEXT_FONT_21 range:redRange];
    [textView setAttributedText:noteStr];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (!_willDisappear) {
        _indexLabel.text = [NSString stringWithFormat:@"%@",self.titleArray[index]];
        NSString *bootmString = [NSString stringWithFormat:@"%d/%ld 病情描述：%@",index+1,self.imageCount,self.commdtion];
        NSMutableAttributedString *noteStr = [[NSMutableAttributedString alloc] initWithString:bootmString];
        NSString *integer = [NSString stringWithFormat:@"%d/%ld",index+1,self.imageCount];
        NSRange redRange = NSMakeRange([[noteStr string] rangeOfString:integer].location, [[noteStr string] rangeOfString:integer].length);
        [noteStr addAttribute:NSFontAttributeName value:TEXT_FONT_21 range:redRange];
        [_textView setAttributedText:noteStr];
    }
