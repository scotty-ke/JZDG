...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        if (_photo.sourceFromAblum) {
//            _imageView.image = [UIImage imageWithCGImage:_photo.asset.thumbnail];
            ALAssetRepresentation* representation = [_photo.asset defaultRepresentation];
...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        _imageView.image = _photo.image;
    }else if(_photo.sourceFromAblum){
        self.scrollEnabled = YES;
        ALAssetRepresentation* representation = [_photo.asset defaultRepresentation];
        _imageView.image = [UIImage imageWithCGImage:[representation fullScreenImage]];
    }else {
